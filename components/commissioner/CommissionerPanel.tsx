...
export function CommissionerPanel() {
  ...
  useEffect(() => {
    if (!enabled) return;
    fetchJSON<{ submissions: Submission[] }>(`/api/payment/submissions?q=${encodeURIComponent(search)}`).then((d) => setSubmissions(d.submissions ?? [])).catch(() => setSubmissions([]));
  }, [enabled, search]);
  ...
  const run = async (url: string, method = "POST", body?: Record<string, unknown>) => {
    ...
    loadCore();
    if (enabled) fetchJSON<{ submissions: Submission[] }>(`/api/payment/submissions?q=${encodeURIComponent(search)}`).then((d) => setSubmissions(d.submissions ?? [])).catch(() => null);
  };
    ...
    <section className="card p-4 space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-white/70">Owner + Staff ops panel. Hidden from public users.</p>
        </div>
      ...
      {enabled && <div className="grid gap-4 lg:grid-cols-2">
        ...
        <form className="space-y-2 border border-white/10 p-3" onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); run("/api/captains", "POST", Object.fromEntries(f.entries())); e.currentTarget.reset(); }}>
          <p className="text-sm font-semibold">Captains</p>
          ...
          <div className="space-y-1 max-h-40 overflow-auto">
            {captains.map((c) => <div key={c.id} className="flex justify-between text-xs"><span>{c.name}</span><button type="button" className="text-danger" onClick={() => run("/api/captains", "DELETE", { captainId: c.id })}>Remove</button></div>)}
          </div>
        </form>
            ...
            {submissions.map((s) => (
              <div key={s.user_id} className="rounded border border-white/10 p-2 text-xs">
                ...
                <div className="mt-1 flex gap-2">
                  <button type="button" className="text-neon" onClick={() => run("/api/payment", "PATCH", { userId: s.user_id, action: "confirm" })}>Confirm</button>
                  <button type="button" className="text-danger" onClick={() => run("/api/payment", "PATCH", { userId: s.user_id, action: "reject" })}>Reject</button>
                </div>
              </div>
            ))}
